import { GSwap, PrivateKeySigner, type GalaChainSigner } from '@gala-chain/gswap-sdk';
...
export class GSwapSDKClient {
  ...
  async getBalance(tokenAddress: string, walletAddress: string): Promise<string> {
    ...
    try {
      // Use the assets service to get user balances
      const assets = await this.gswap.assets.getUserAssets(walletAddress, 1, 100);
  ...
  async getUserAssets(walletAddress: string): Promise<any> {
    ...
    try {
      return await this.gswap.assets.getUserAssets(walletAddress, 1, 100);
    } catch (error) {
  ...
  async executeSwap(params: SwapParams): Promise<string> {
    ...
    try {
      ...
      // Return transaction hash or identifier
      if (swapResult && typeof swapResult === 'object' && 'transactionHash' in swapResult) {
        return (swapResult as any).transactionHash;
      } else if (swapResult && typeof swapResult === 'string') {
        return swapResult;
      } else {
        // Generate a mock transaction hash for now
        const timestamp = Date.now().toString(16);
        const random = Math.random().toString(16).substring(2, 10);
        return `0xgswap${timestamp}${random}`.padEnd(66, '0');
      }
    } catch (error: any) {
